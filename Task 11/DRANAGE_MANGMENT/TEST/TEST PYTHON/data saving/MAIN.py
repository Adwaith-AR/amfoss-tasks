...
board1_ws = board1_wb.active
board2_ws = board2_wb.active
board3_ws = board3_wb.active

file_path_primary='/home/adwaith-a/Documents/Adwaith .A/Adwaith .A/coding/WORK/DRANAGE_MANGMENT/TEST/TEST PYTHON/data saving/ test/'
date1 = datetime.now().strftime("%d %Y")
hour1 = datetime.now().strftime("%I:%M %p") 
try:
    file_path_temparery='/home/adwaith-a/Documents/Adwaith .A/Adwaith .A/coding/WORK/DRANAGE_MANGMENT/TEST/TEST PYTHON/data saving/ test/'+date1
    os.mkdir(file_path_temparery)
    file_path_temparery='/home/adwaith-a/Documents/Adwaith .A/Adwaith .A/coding/WORK/DRANAGE_MANGMENT/TEST/TEST PYTHON/data saving/ test/'+date1+'/'+hour1
    os.mkdir(file_path_temparery)
    print(file_path_temparery)
except:
    pass

last_creation_time_hour = datetime.now()
last_creation_time_day = datetime.now()
...
def save_data(board,data):
    global last_creation_time_hour 
    global last_creation_time_day
    global file_path_primary
    global file_path_temparery
    board1_data = [["Water level", "obstacles","Gas level"]]
    board2_data = [["Water level", "obstacles","Gas level"]]
    board3_data = [["Water level", "obstacles","Gas level"]]
    if board == 1:
        board1_data.append(data)
    if board == 2:
        board2_data.append(data)
    if board == 3:
        board3_data.append(data)
    # Format the folder name as "YYYY-MM-DD_HH-MM-SS"
    folder_name = datetime.now().strftime("%I:%M %p  %d %Y")

    date = datetime.now().strftime("%d %Y") 
    hour = datetime.now().strftime("%I:%M %p") 
    
    
    current_time_hour = datetime.now()

    
    current_time_day = datetime.now()

    # Check if 1 hour has passed since the last folder creation
    if current_time_hour - last_creation_time_hour >= timedelta(hours=1):
        # Create a new folder
        try:
            os.makedirs(file_path_primary+date+'/'+hour)
            file_path_temparery=file_path_primary+date+'/'+hour+'/'
        except:
            pass
        last_creation_time_hour = current_time_hour  # Update the last creation time

    elif current_time_day - last_creation_time_day >= timedelta(minutes=1):
        try:
            os.makedirs(file_path_primary+'/'+date)
            file_path_temparery=file_path_primary+date+'/'+hour+'/'
            os.makedirs(file_path_primary)
        except:
            pass
        for row in board1_data:
            board1_ws.append(row)
        for row in board2_data:
            board2_ws.append(row)
        for row in board2_data:
            board3_ws.append(row)    
        board1_wb.save(file_path_temparery+"/board1.xlsx")
        board2_wb.save(file_path_temparery+"/board2.xlsx")
        board3_wb.save(file_path_temparery+"/board3.xlsx")
      

        
        last_creation_time_day = current_time_day  # Update the last creation time
