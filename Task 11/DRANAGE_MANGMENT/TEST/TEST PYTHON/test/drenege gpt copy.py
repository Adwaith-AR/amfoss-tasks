...
class MQTTApp:
    def __init__(self, root):
        ...
        self.root.geometry("800x400")  # Increased window size to accommodate more bars
        ...
        self.cm_water_bard4 = 0  # Added variable for bard4 water level
        self.cm_obstacles_bard4 = 0  # Added variable for bard4 obstacles level
        ...
        self.root.grid_columnconfigure(2, weight=1, minsize=250)  # Added column for bard4
        ...
        self.bard2_title = tk.Label(self.bard2_frame, text="Node2 Data", font=("Helvetica", 12, "bold"))
        self.bard2_title.pack(pady=5)

        # Water Level Progress Bar for bard2
        self.water_level_label = tk.Label(self.bard2_frame, text="Water Level (bard2): 0%")
        self.water_level_label.pack()
        self.water_level_bar = ttk.Progressbar(self.bard2_frame, orient="horizontal", length=200, mode="determinate")
        self.water_level_bar.pack()

        # Obstacles Progress Bar for bard2
        self.obstacles_label = tk.Label(self.bard2_frame, text="Obstacles (bard2): 0%")
        self.obstacles_label.pack()
        self.obstacles_bar = ttk.Progressbar(self.bard2_frame, orient="horizontal", length=200, mode="determinate")
        self.obstacles_bar.pack()

        # Frame for bard1 data (middle section)
        self.bard1_frame = tk.Frame(self.root)
        self.bard1_frame.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")

        # Add bard1 label
        self.bard1_label = tk.Label(self.bard1_frame, text="Node 1  Data", font=("Helvetica", 12, "bold"))
        ...
        self.water_level_bar_bard1 = ttk.Progressbar(self.bard1_frame, orient="horizontal", length=200, mode="determinate")
        ...
        self.obstacles_bar_bard1 = ttk.Progressbar(self.bard1_frame, orient="horizontal", length=200, mode="determinate")
        self.obstacles_bar_bard1.pack()

        # Frame for bard3 data (right section)
        self.bard3_frame = tk.Frame(self.root)
        self.bard3_frame.grid(row=1, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")

        # Title for bard3 section
        self.bard3_title = tk.Label(self.bard3_frame, text="Node3 Data", font=("Helvetica", 12, "bold"))
        ...
        self.water_level_bar_bard3 = ttk.Progressbar(self.bard3_frame, orient="horizontal", length=200, mode="determinate")
        ...
        self.obstacles_bar_bard3 = ttk.Progressbar(self.bard3_frame, orient="horizontal", length=200, mode="determinate")
        self.obstacles_bar_bard3.pack()

        # Frame for bard4 data (new section)
        self.bard4_frame = tk.Frame(self.root)
        self.bard4_frame.grid(row=1, column=2, padx=10, pady=10, sticky="nsew")  # Added new column for bard4

        # Title for bard4 section
        self.bard4_title = tk.Label(self.bard4_frame, text="Node4 Data", font=("Helvetica", 12, "bold"))
        self.bard4_title.pack(pady=5)

        # Water Level Progress Bar for bard4
        self.water_level_label_bard4 = tk.Label(self.bard4_frame, text="Water Level (bard4): 0%")
        self.water_level_label_bard4.pack()
        self.water_level_bar_bard4 = ttk.Progressbar(self.bard4_frame, orient="horizontal", length=200, mode="determinate")
        self.water_level_bar_bard4.pack()

        # Obstacles Progress Bar for bard4
        self.obstacles_label_bard4 = tk.Label(self.bard4_frame, text="Obstacles (bard4): 0%")
        self.obstacles_label_bard4.pack()
        self.obstacles_bar_bard4 = ttk.Progressbar(self.bard4_frame, orient="horizontal", length=200, mode="determinate")
        self.obstacles_bar_bard4.pack()
        ...
        self.client.connect("broker.hivemq.com", port=1883)
    ...
    def msg_to_work(self, msg):
        ...
        # Handle bard4 messages (added logic for bard4)
        if "bard4" in msg:
            msg = msg.replace('bard4', '')  # Correctly remove 'bard4' from the message
            if "water level: " in msg:
                msg = msg.replace(' water level: ', '')
                try:
                    self.cm_water_bard4 = int(int(msg) / 10)
                    water_level = (int(msg) / self.water_level_max_read_value) * 100
                    self.update_progress_bar_bard4(water_level, "Water Level (bard4)", self.cm_water_bard4)
                except ValueError:
                    print("Invalid water level value")
                ...
                msg = msg.replace(' obstacles: ', '')
                try:
                    self.cm_obstacles_bard4 = int(int(msg) / 10)
                    obstacles = (int(msg) / self.obstacles_level_max_read_value) * 100
                    self.update_progress_bar_bard4(obstacles, "Obstacles (bard4)", self.cm_obstacles_bard4)
                except ValueError:
                    print("Invalid obstacles value")

    # Tkinter progress bar update function for bard2
    ...
    # Tkinter progress bar update function for bard1
    ...
    # Tkinter progress bar update function for bard3
    def update_progress_bar_bard3(self, value, label, cm):
        reversed_value = 100 - value
        if label == "Water Level (bard3)":
            self.water_level_bar_bard3["value"] = reversed_value
            self.water_level_label_bard3["text"] = f"Water Level (bard3): {cm}%"
        elif label == "Obstacles (bard3)":
            self.obstacles_bar_bard3["value"] = reversed_value
            self.obstacles_label_bard3["text"] = f"Obstacles (bard3): {cm}%"

    # Tkinter progress bar update function for bard4 (newly added)
    def update_progress_bar_bard4(self, value, label, cm):
        reversed_value = 100 - value
        if label == "Water Level (bard4)":
            self.water_level_bar_bard4["value"] = reversed_value
            self.water_level_label_bard4["text"] = f"Water Level (bard4): {cm}%"
        elif label == "Obstacles (bard4)":
            self.obstacles_bar_bard4["value"] = reversed_value
            self.obstacles_label_bard4["text"] = f"Obstacles (bard4): {cm}%"
