...
class MQTTApp:
    def __init__(self, root):
        ...
        self.gas_level_max_read_value = 100  # Assuming gas level is in percentage (0-100)
        
        # Existing variables for sensors
        self.cm_water = 0
        self.cm_obstacles = 0
        ...
        self.cm_gas = 0  # For gas sensor

        self.connected = False

        # Grid configuration to make layout more compact
        self.root.grid_rowconfigure(0, weight=1, minsize=15) # Allow row 0 to expand but with a minimum height
        self.root.grid_columnconfigure(0, weight=1, minsize=20)  # Allow column 0 to expand with a minimum width
        self.root.grid_columnconfigure(1, weight=1, minsize=20)  # Allow column 1 to expand with a minimum width
        ...
        # Gas Level Progress Bar for bard2
        self.gas_label = tk.Label(self.bard2_frame, text="Gas Level : 0%")
        self.gas_label.pack()
        ...
        # Gas Level Progress Bar for bard1
        self.gas_label_bard1 = tk.Label(self.bard1_frame, text="Gas Level (bard1): 0%")
        ...
        # Gas Level Progress Bar for bard3
        self.gas_label_bard3 = tk.Label(self.bard3_frame, text="Gas Level (bard3): 0%")
    ...
    def msg_to_work(self, msg):
        ...
        if "bard2" in msg:
            ...
            # Handle gas sensor data for bard2
            elif "gas level: " in msg:
                msg = msg.replace(' gas level: ', '')
                try:
                    self.cm_gas = int(int(msg) / 10)
                    gas_level = (int(msg) / self.gas_level_max_read_value) * 100
                    self.update_progress_bar_gas(gas_level, "Gas Level", self.cm_gas)
                except ValueError:
                    print("Invalid gas level value")

        # Similar handling for bard1 and bard3 with respect to gas sensor data
        # Repeating the similar process for other nodes (bard1, bard3)...
        # For bard1 and bard3, update functions can be added in the same manner as done above

    # Update progress bar for gas sensor data for bard2
    def update_progress_bar_gas(self, value, label, cm):
        # Reverse the value by subtracting from 100
        reversed_value = 100 - value
        if label == "Gas Level":
            self.gas_bar["value"] = reversed_value
            self.gas_label["text"] = f"Gas Level (bard2): {cm}%"
